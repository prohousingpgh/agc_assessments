...
- "instructions" defines the algorithms (regression, decision tree, etc) to use for each model group. Currently, the final ensemble considers linear regression ("mra"), spatial models ("local_area", "spatial_lag_area"), and decision tree models ("lightgbm","xgboost") across all model groups. The "assessor" model is run only as a benchmark against existing assessments and is not included in the final ensemble.
- The "main" ensemble is a median ensemble optimized by OpenAVMKit. It starts with the five listed candidate models, repeatedly drops the weakest model-level candidate, and keeps the subset with the best lookback error score. The final valuation for each parcel is the median prediction from the selected subset. OpenAVMKit writes the selected subset to `out/models/<model_group>/main/ensemble/ensemble_meta.json`.
